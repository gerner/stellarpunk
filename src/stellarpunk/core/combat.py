...
class AttackOrder(movement.AbstractSteeringOrder, core.SectorEntityObserver):
    ...
    def act(self, dt:float) -> None:
        ...
        if distance > self.distance_max:
            self.standoff_order = movement.PursueOrder(self.target, self.ship, self.gamestate, arrival_distance=self.distance_max-0.2*(self.distance_max-self.distance_min))
            self._add_child(self.standoff_order)
            return
        elif distance < self.distance_min:
            self.standoff_order = movement.EvadeOrder(self.target, self.ship, self.gamestate, escape_distance=self.distance_min+0.2*(self.distance_max-self.distance_min))
            self._add_child(self.standoff_order)
            return
        ...
        accelerate_time = collision.accelerate_to(self.ship.phys, cymunk.Vec2d(target_velocity), dt, self.ship.max_speed(), self.ship.max_torque, self.ship.max_thrust, self.ship.max_fine_thrust)

        # TODO: make attacks

        self.gamestate.schedule_order(self.gamestate.timestamp + min(accelerate_time, 1/10), self)
